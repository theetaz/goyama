...
//
// First pass: crops only. Each crop record is split across three tables —
// the scalar columns live on `crop`, `names` and `description` go into
// `translation`, and `aliases` go into `entity_alias`. The entire load is
// idempotent: running it twice on the same corpus leaves the DB unchanged.
//
// Usage:
//
...
//	    go run ./cmd/seed
//
// By default the loader walks ../../corpus/seed/crops relative to the
// services/api working directory; override with --corpus.
package main
...
func run() error {
	corpus := flag.String("corpus", "../../corpus/seed/crops", "path to the crops seed directory")
	flag.Parse()
	...
	if err != nil {
		return fmt.Errorf("glob corpus: %w", err)
	}
	if len(files) == 0 {
		return fmt.Errorf("no records under %s", *corpus)
	}
	sort.Strings(files)
